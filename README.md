...
### 同仁請假登記
- 自動辨識請假類型訊息
- 支援假別標註：產假、產檢假、病假、喪假、事假、補休、特休、公假、婚假等
- 有註明假別時顯示為「姓名+假別+時間」（例如：張三產檢假0900-1000）
- 未註明假別時預設顯示為「姓名+請休+時間」
...
```
登記 3月15日下午兩點到四點，在A棟301會議室，總額研商會議
主持人：劉司長
出席人員：理事長(O) 幕僚 張三
會議連結：https://reurl.cc/ABCDE
會議號/密碼：1234 567 8910/0315
```
...
支援的假別：產假、產檢假、病假、喪假、事假、補休、特休、公假、婚假。
未指定假別時預設為「請休」。
...
- **Synology NAS**（Plus 系列，或其他支援 Docker / Container Manager 的NAS）
...
4. 在同一個資料夾內建立 `reports` 子資料夾（用於存放每日報表）：
